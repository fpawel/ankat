...
func (x* app) mainWork() uiworks.Work{
	...
	sleep10 := delay(x.workCtrl, "Задержка 10с", time.Second * 10)
	...
	return uiworks.L("Настройка ДАК-М",
		uiworks.L("Продувка воздухом",
			...
			x.workCtrl.WorkDelay("Задержка 10 с", func() time.Duration {
		...
		uiworks.L("Продувка воздухом2",
			...
			x.workCtrl.WorkDelay("Задержка 10 с2", func() time.Duration {
				return 10 * time.Second
			}, nil),
		),
	)
